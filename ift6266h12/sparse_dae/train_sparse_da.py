...
from special_costs import SampledMeanSquaredReconstructionError
from sparse_dae import SparseDenoisingAutoencoder
from sparse_dae import Rectify
from toy_datasets import ToyDatasetDense
...
def get_dataset_toy_dense():
    ...
    trainset = ToyDatasetDense()
    testset = ToyDatasetDense()
    return trainset, testset

def get_dataset_toy_sparse():
    trainset = ToyDatasetSparse()
    testset = ToyDatasetSparse()
    return trainset, testset

def get_dataset_terry():
    terry =  Terry()
    return terry.trainset, terry.testset
...
#######################################################################
def get_sparse_denoising_autoencoder(structure):
    n_input, n_output = structure
    curruptor = BinomialCorruptor(corruption_level=0.1)
    config = {
        'corruptor': curruptor,
        'nhid': n_output,
        'nvis': n_input,
    ...
    return SparseDenoisingAutoencoder(**config)
...
def main():
    ...
    if options.dataset == 'toy_dense':
        trainset, testset = get_dataset_toy_dense()
        SAVE_MODEL = False
    elif options.dataset == 'cifar10':
        trainset, testset = get_dataset_cifar10()
        SAVE_MODEL = True
    elif options.dataset == 'toy_sparse':
        trainset, testset = get_dataset_toy_sparse()
        SAVE_MODEL = True
    elif options.dataset == 'terry':
        trainset, testset = get_dataset_terry()
        SAVE_MODEL = False
    ...
    """
    # layer 0:
    layers.append(get_sparse_denoising_autoencoder(structure[0]))
    
    # layer 1: 
    #layers.append(get_sparse_autoencoder(structure[1]))

    # layer 2: 
    #layers.append(get_sparse_autoencoder(structure[2]))

    # layer 3: Supervised layer, not implemented for now.
    ...
    layer_trainers = []
    """
    #layer_trainers.append(get_layer_trainer_sparse_autoencoder(layers[0], trainset))
    ...
    layer_trainers.append(get_layer_trainer_sgd_autoencoder(layers[0], trainset[0]))
    ...
    """
    layer_trainers.append(get_layer_trainer_sparse_denoising_autoencoder(layers[0], trainset[0]))
    #layer_trainers.append(get_layer_trainer_sparse_autoencoder(layers[1], trainset[1]))
    #layer_trainers.append(get_layer_trainer_sparse_autoencoder(layers[2], trainset[2]))
